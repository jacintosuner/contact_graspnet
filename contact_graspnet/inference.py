...
def inference(global_config, checkpoint_dir, input_paths, results_path, K=None, local_regions=True, skip_border_objects=False, filter_grasps=True, segmap_id=None, z_range=[0.2,1.8], forward_passes=1):
    ...
    initial_pcd = np.load(os.path.join(input_paths, "initial_pcd.npy"))
    initial_pcd_seg = np.load(os.path.join(input_paths, "initial_pcd_seg.npy"))
    num_objs = len(np.unique(initial_pcd_seg)) - 1  # background is not an object, so exclude it
...
if __name__ == "__main__":
    ...
    # python3 contact_graspnet/inference.py --np_path initial_pcd.npy --filter_grasps --results_path /home/jacinto/franka-panda-execution/contact_graspnet/contact_graspnet/out --ckpt_dir ~/robot-grasp/third_party/contact_graspnet/checkpoints/scene_test_2048_bs3_hor_sigma_0025
